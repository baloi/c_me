...
{
    // We need to choose the path separator properly based on which platform we're
    // running on, since Windows uses a different separator tham most systems.
#ifdef _WIN32
    const char PATH_SEP = '\\';
#else
    const char PATH_SEP = '/';
    ...
    // We give it stattic lifetime so that we'll only need to call
